...
def main(directory=WORKING_DIRECTORY, linestyle=LINESTYLE):
    ...
    temp_range = round(temperature_data[0] - temperature_data[width - 1], 2)
    linsteps = int(round(temp_range/0.01, 2)) + 1
    header_list = np.around(np.linspace(temperature_data[0],
                                        temperature_data[width - 1], linsteps), 2)
    indices = []
