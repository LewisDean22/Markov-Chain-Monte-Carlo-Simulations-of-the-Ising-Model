...
def find_new_state_from_cluster_point(spins_state, spin_coordinate, J, T):
    ...
    while coordinates_to_test:
        ...
        # plot_state(new_state, 'flipped')
        count += 1
    print("Cluster size: {}".format(len(cluster_coordinates)))
    return new_state


def find_cluster_progression(temperature, length, number_of_iterations, J):
    ...
    # new_state = find_new_state_from_cluster_point(initial_state, spin_coordinate, J_CONST, TEMPERATURE)
    pvar(new_state)
    pvar(total_magnetisation)
    pvar(total_magnetisation_abs)
    pvar(total_magnetisation_squared)
    pvar(total_magnetisation_4)
    pvar(total_energy)
    pvar(total_energy_squared)
    return new_state, total_magnetisation, total_magnetisation_abs, total_magnetisation_squared, total_magnetisation_4, total_energy, total_energy_squared
