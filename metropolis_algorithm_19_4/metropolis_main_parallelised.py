...
def parallel_temp_steps(temp, length=LENGTH):
    ...
    equilibrium_state, arbitrary_energies, arbitrary_magnetisations = find_state_progression(initial_state, temp, J_CONST, length, NUMBER_OF_ITERATIONS, TRANSIENT_PERIOD)
    ...
    return [temp, total_magnetisation, total_magnetisation_abs,
...
def main(n_cores=NUMBER_OF_CORES_USED): 
    ...
    for index, array in enumerate(results):
        (temp, total_magnetisation, total_magnetisation_abs,
         total_magnetisation_squared, total_magnetisation_4,
         total_energy, total_energy_squared) = array
    ...
    end_main = tm.time()
    print(Fore.RESET + "Run time = {:.0f} s".format(end_main-start_main))
