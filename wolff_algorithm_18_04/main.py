...
NORM = 1 / (LENGTH**2 * NUMBER_OF_ITERATIONS)
...
def main():
    ...
    while temperature >= MINIMUM_TEMPERATURE:
        ...
        (equilibrium_state,
         a, b, c, d, e, f) = find_cluster_progression(initial_state, temperature,
                                                      LENGTH,
                                                      TRANSIENT_PERIOD,
                                                      J_CONST)
        ...
        (g, total_magnetisation, 
         ...
         total_energy, 
         total_energy_squared) = find_cluster_progression(equilibrium_state,
                                                          temperature,
                                                          LENGTH,
        ...
        print(total_magnetisation * NORM)
    ...
    np.savetxt("Magnetisation_{0}x{0}_MC_{1}_wolff.csv".format(LENGTH, NUMBER_OF_ITERATIONS), magnetisation_data, delimiter=',')
    np.savetxt("Energy_{0}x{0}_MC_{1}_wolff.csv".format(LENGTH, NUMBER_OF_ITERATIONS), energy_data, delimiter=',')
